  ...
  group('Configuration Files Validation', () {
    ...
    test('README.md has valid markdown structure', () {
      ...
      if (readmeFile.existsSync()) {
        ...
        expect(RegExp(r'^#\s+').hasMatch(content), isTrue,
    ...
    test('.qodo.toml exists and is valid TOML format', () {
      ...
      expect(qodoFile.existsSync(), isTrue,
      ...
      if (qodoFile.existsSync()) {
        ...
        expect(content, isNotEmpty,
            reason: '.qodo.toml should not be empty');

        // Check for TOML section headers
        expect(RegExp(r'\[.+\]').hasMatch(content), isTrue,
            reason: '.qodo.toml should contain TOML sections');

        // Check for key-value pairs
        expect(RegExp(r'^\w+\s*=\s*.+', multiLine: true).hasMatch(content), isTrue,
            reason: '.qodo.toml should contain key-value pairs');

        // Verify expected sections
        expect(content, contains('[pr_code_suggestions]'),
            reason: '.qodo.toml should have pr_code_suggestions section');
        expect(content, contains('[rag_arguments]'),
    ...
    test('.qodo.toml has required configuration keys', () {
      ...
      if (qodoFile.existsSync()) {
        ...
        expect(content, contains('include_globs'),
            reason: '.qodo.toml should specify include_globs');
        expect(content, contains('exclude_globs'),
            reason: '.qodo.toml should specify exclude_globs');
        expect(content, contains('max_file_size_kb'),
            reason: '.qodo.toml should specify max_file_size_kb');
      }
    });

    test('GitHub bug report template exists and is valid', () {
      final bugReportFile = File(path.join(
          projectRoot.path, '.github', 'ISSUE_TEMPLATE', 'bug_report.md'));

      expect(bugReportFile.existsSync(), isTrue,
      ...
      if (bugReportFile.existsSync()) {
        ...
        expect(content.toLowerCase(), contains('describe the bug'),
            reason: 'Bug report should have "Describe the bug" section');
        expect(content.toLowerCase(), contains('to reproduce'),
    ...
    test('android/app/build.gradle.kts exists and has valid structure', () {
      ...
      if (buildFile.existsSync()) {
        ...
        expect(content, contains('targetSdk'),
    ...
    test('build.gradle.kts has correct package name', () {
      ...
      if (buildFile.existsSync()) {
        ...
        expect(content, contains('com.omeritzics.updatium'),
    ...
    test('build.gradle.kts has version code calculation logic', () {
      ...
      if (buildFile.existsSync()) {
        ...
        expect(content, contains('versionCode'),
            reason: 'build.gradle.kts should have versionCode configuration');
        expect(content, contains('abiCodes'),
            reason: 'build.gradle.kts should define ABI codes for version calculation');
        expect(content, contains('androidComponents'),
            reason: 'build.gradle.kts should use androidComponents for version calculation');
      }
    });
    ...
    test('build.gradle.kts configures ABI splits correctly', () {
      ...
      if (buildFile.existsSync()) {
        ...
        expect(content, contains('splits {'),
  ...
  group('GitHub Workflow Files Validation', () {
    ...
    setUpAll(() {
      ...
      projectRoot = testDir.path.endsWith('test')
          ? testDir.parent
          : testDir;
      workflowsDir = Directory(path.join(
          projectRoot.path, '.github', 'workflows'));
    });
    ...
    test('ci.yml workflow exists and has valid structure', () {
      ...
      if (ciFile.existsSync()) {
        ...
        expect(content, contains('name:'),
            reason: 'ci.yml should have a name');
        expect(content, contains('on:'),
            reason: 'ci.yml should define triggers');
        expect(content, contains('jobs:'),
            reason: 'ci.yml should define jobs');
        expect(content, contains('build_artifact'),
    ...
    test('ci.yml builds APKs with proper configuration', () {
      ...
      if (ciFile.existsSync()) {
        ...
        expect(content, contains('--split-per-abi'),
    ...
    test('nightly.yml workflow exists and has valid structure', () {
      ...
      expect(nightlyFile.existsSync(), isTrue,
    ...
    test('nightly.yml creates GitHub releases', () {
      ...
      if (nightlyFile.existsSync()) {
        ...
        expect(content, contains('action-gh-release'),
            reason: 'nightly.yml should create GitHub releases');
        expect(content, contains('nightly-build'),
    ...
    test('release.yml handles signing configuration', () {
      ...
      if (releaseFile.existsSync()) {
        ...
        expect(content, contains('Decode Keystore'),
    ...
    test('release.yml builds obfuscated release APKs', () {
      ...
      if (releaseFile.existsSync()) {
        ...
        expect(content, contains('--obfuscate'),
    ...
    test('release.yml cleans up sensitive files', () {
      ...
      if (releaseFile.existsSync()) {
        ...
        expect(content.toLowerCase(), contains('keystore'),
    ...
    test('all workflow files use consistent Flutter version', () {
      ...
      for (final file in workflowFiles) {
        ...
        if (flutterVersion == null) {
          ...
          expect(version, equals(flutterVersion),
              reason: 'All workflows should use the same Flutter version (found $version vs $flutterVersion in ${path.basename(file.path)})');
        }
            }
    });
  });

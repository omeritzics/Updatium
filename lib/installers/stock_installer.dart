...
import 'package:updatium/utils/string_utils.dart';
...
class StockInstaller extends Installer {
  ...
  Future<bool> canInstallSilently(App app) async {
    ...
    if (installerPackageName == null ||
        !isUpdatiumVariant(installerPackageName)) {
      // If we did not install the app, silent install is not possible
      AppLogger.info(
        'App will not be installed silently: Updatium is not the installing package (current installer: $installerPackageName): ${app.id}',
      );
      return false;
    }
